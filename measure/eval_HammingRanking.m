function [ROC, Precision, mAP]=eval_HammingRanking(Y, tY, traingnd, testgnd, range)
%
% ROC - 
%
%
...
if isnumeric(Y),
    sim = Y*tY'; % Similarity of traning data and testing data, ntr by d, d by nte, ntr by nte
elseif iscell(Y),
    sim = zeros(length(traingnd), length(testgnd));
    start = 1; 
    for i = 1:length(Y),
        stop = start + size(Y{i}, 1) - 1;
        sim(start:stop, :) = Y{i}*tY{i}';
        start = stop + 1;
    end
else
    error('eval_HammingRanking: Unknown data type');
end
