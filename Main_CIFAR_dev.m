function [results] = Main_CIFAR(prefix, maxiter)
% clc; 
close all;  
dbstop if error
...
if ~exist('steprange', 'var'), 
    %steprange = [10, 20, 30, 40, 50, 60, 70, 80];
    steprange = [10, 20, 50, 100, 150, 200, 300, 400, 500, 1000];
end
...
switch(prefix)
    case 'unsup_'
        ...
        %methods = {'ITQ', 'ITQ-SS'};
        methods = {'ITQ', 'ITQ-SS', 'PCA', 'PCA-SS'};
        isunsup = true;
    case 'unsupk_'
        %***** Unsupervised kernel methods
        prefix = 'unsupk_';
        %methods = {'ITQ-SS', 'KPCA-ITQ', 'KPCA-RR', 'KPCA-Direct', 'KPCA-ITQ-SS', 'KPCA-RR-SS', 'KPCA-Direct-SS', 'SKLSH'};
        methods = {'SKLSH'};
        isunsup = true;
    case 'sup_'
        %***** Supervised methods
        prefix = 'sup_';
        methods = {'CCA-Direct', 'CCA-ITQ', 'CCA-RR', 'CCA-ITQ-SS', 'CCA-RR-SS', 'CCA-Direct-SS', 'ITQ-SS'};
    ...
    case 'supk_'
        ...
        %methods = {'ITQ-SS', 'KCCA-RR-SS', 'KPCA-ITQ-SS'};
        isunsup = false;
    otherwise
...
opt.gamma = optSigma(traindata, 15);
...
for mi = 1:length(methods),
    ...
    for s = steprange,
        ...
        k = k + 1;
        result(k).r = s;  % sample ratio
        result(k).numbit = r;  % bit length
...
save(fullfile(outdir, [prefix 'result_dev.mat']), 'results');
